...
export class VideoRTC extends HTMLElement {
  constructor() {
     ...
     */
    this.onmessage = null;
    // IntersectionObserver for visibility check
  ...
  codecs(isSupported) {
    return this.CODECS
      .filter(codec => this.media.includes(codec.indexOf('vc1') > 0 ? 'video' : 'audio'))
  ...
  disconnectedCallback() {
    if (this.background || this.disconnectTID)
      return;
    ...
    //   document.removeEventListener('visibilitychange', this.onDocumentVisibilityChange.bind(this));
    // }
  ...
  onwebrtc() {
    ...
    pc.addEventListener('icecandidate', (ev) => {
      if (ev.candidate && this.mode.includes('webrtc/tcp') && ev.candidate.protocol === 'udp')
        return;
    ...
    this.onmessage.webrtc = (msg) => {
      switch (msg.type) {
        case 'webrtc/candidate':
          if (this.mode.includes('webrtc/tcp') && msg.value.indexOf(' udp ') > 0)
            return;
  ...
  onpcvideo(video2) {
    if (this.pc) {
      ...
      if (this.mseCodecs.includes('hvc1.'))
        msePriority += 0x230;
      if (this.mseCodecs.includes('avc1.'))
        msePriority += 0x210;
      if (this.mseCodecs.includes('mp4a.'))
        msePriority += 0x101;
  ...
  onVideoError(ev) {
    ...
    // debugger;
    if (this.ws)
      this.ws.close(); // run reconnect for broken MSE stream
  }
